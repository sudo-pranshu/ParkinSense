...
// ---- Finger-presence gating (MAX30102 INT pin not wired, so this is a
// software poll-based gate rather than a true hardware wake) ----
// No finger:   IR ~1600, RED ~600
// With finger: IR ~109000, RED ~80000
// Threshold sits roughly midway, well clear of both, so noise near either
// end can't cause a false trigger.
const uint32_t FINGER_THRESHOLD_IR = 50000;
const uint8_t FINGER_DEBOUNCE_COUNT = 3;       // consecutive polls required to switch state
const uint32_t FINGER_POLL_INTERVAL_MS = 250;  // how often we check IR against threshold, asleep or active
...
// ---- Finger-presence gate state ----
bool systemActive = false;
uint8_t fingerAboveCount = 0;
uint8_t fingerBelowCount = 0;
uint32_t lastFingerCheckMs = 0;
...
// Lightweight single-sample IR read, used only for the low-power
// finger-presence poll while the system is asleep. Cheaper than draining
// the whole FIFO in the main PPG block - we just need one fresh value to
// threshold against.
uint32_t quickReadIR()
{
    return max30102.getIR();
}
...
void loop()
{
    ...
    // Only the IR threshold decides sleep/wake. IMU stays completely
    // untouched - it neither drives nor blocks this decision, and once
    // systemActive is true, everything below runs exactly as before.
    if (!systemActive)
    {
        if (spo2Ready && (nowMs - lastFingerCheckMs) >= FINGER_POLL_INTERVAL_MS)
        {
            lastFingerCheckMs = nowMs;
            uint32_t irVal = quickReadIR();

            if (irVal > FINGER_THRESHOLD_IR)
            {
                fingerAboveCount++;
            }
            else
            ...
            if (fingerAboveCount >= FINGER_DEBOUNCE_COUNT)
            {
                systemActive = true;
                fingerAboveCount = 0;
                fingerBelowCount = 0;
                // Reset timing so we don't fire a burst of catch-up samples
                // the instant we wake up.
                lastSampleUs = micros();
                lastPPGMs = nowMs;
                batchIndex = 0;
                Serial.println("Finger detected -> waking up, starting transmission");
            }
        }
        delay(10); // let the CPU idle between polls; keeps this branch low power
        return;    // skip IMU sampling, PPG batching, and BLE notify while asleep
    }
    else
    {
        // While active, periodically confirm the finger is still present.
        // Reuses latestIR from the PPG block below - no extra I2C traffic.
        if ((nowMs - lastFingerCheckMs) >= FINGER_POLL_INTERVAL_MS)
        {
            lastFingerCheckMs = nowMs;

            if (latestIR < FINGER_THRESHOLD_IR)
            {
                fingerBelowCount++;
            }
            else
            ...
            if (fingerBelowCount >= FINGER_DEBOUNCE_COUNT)
            {
                systemActive = false;
                fingerBelowCount = 0;
                batchIndex = 0; // discard any partial batch
                Serial.println("Finger removed -> going back to sleep");
                return;
            }
        }
    }
